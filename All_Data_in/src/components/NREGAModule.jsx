...
const NATIONAL_TREND = [
  ...
  { year: "2020-21", personDays: 389, budget: 111500, expenditure: 111170, households: 11.19 },
...
const NregaTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "#0d0d0d", border: "1px solid #00c896", padding: "10px 14px", borderRadius: 4, fontFamily: "'IBM Plex Mono', monospace", fontSize: 11 }}>
      ...
      {payload.map((p, i) => <p key={i} style={{ color: "#e0e0e0", margin: "2px 0" }}>{p.name}: {typeof p.value === "number" ? p.value.toLocaleString("en-IN") : p.value}{p.dataKey === "personDays" ? " crore person-days" : ""}{p.dataKey === "expenditure" ? " crore ₹" : ""}</p>)}
    </div>
  );
};
...
const StateTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: "#0d0d0d", border: `1px solid ${d.color}`, padding: "10px 14px", borderRadius: 4, fontFamily: "'IBM Plex Mono', monospace", fontSize: 11 }}>
      ...
      <p style={{ color: d.days >= 50 ? "#00c896" : d.days >= 35 ? "#ffaa00" : "#ff4444", margin: "4px 0", fontSize: 10 }}>{d.days >= 50 ? "✓ ABOVE NATIONAL AVG" : d.days >= 35 ? "↗ MODERATE" : "⚠ UNDERPERFORMING"}</p>
    </div>
  );
};
  ...
  return (
    <div style={{ background: "#080808", color: "#e0e0e0", fontFamily: "'IBM Plex Sans', sans-serif", borderTop: "1px solid #1a1a1a" }}>
      ...
      <div style={{ padding: "28px 40px 20px", borderBottom: "1px solid #1a1a1a", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 36, letterSpacing: 4, color: "#88ffcc", lineHeight: 1 }}>MGNREGA UTILISATION</div>
          <div style={{ fontSize: 12, color: "#555", letterSpacing: 3, marginTop: 4 }}>100 DAYS GUARANTEED · STATE-WISE DELIVERY · FUND GAPS · WORKER DELETIONS</div>
        </div>
        <div style={{ fontSize: 10, color: "#333", fontFamily: "'IBM Plex Mono', monospace", letterSpacing: 2, textAlign: "right" }}>SOURCE · MINISTRY OF RURAL DEVELOPMENT<br />NREGASOFT · LIBTECH INDIA 2024 · RS Q.1222</div>
      </div>
      ...
      <div style={{ background: "#001a0f", borderBottom: "1px solid #003322", padding: "10px 40px", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: "#448866", letterSpacing: 1, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        MGNREGA guarantees every rural household 100 days of paid work/year. National average actually delivered: ~47 days. Only 2 of 36 states had positive fund balance as of April 2024.
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #1a1a1a", padding: "0 40px" }}>
        {[
          ...
          <button key={t.id} onClick={() => handleTabChange(t.id)} style={{ background: "none", border: "none", borderBottom: tab === t.id ? "2px solid #88ffcc" : "2px solid transparent", color: tab === t.id ? "#88ffcc" : "#444", padding: "14px 20px", cursor: "pointer", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, letterSpacing: 2, marginBottom: -1, transition: "all 0.15s" }}>{t.label}</button>
        ))}
      </div>
      ...
      <div style={{ padding: "32px 40px" }}>
        ...
        {tab === "overview" && (
          <>
            <div style={{ fontSize: 11, color: "#444", letterSpacing: 2, marginBottom: 6 }}>PERSON-DAYS GENERATED · NATIONAL · 2019-20 TO 2023-24</div>
            <div style={{ fontSize: 10, color: "#224433", marginBottom: 20, fontFamily: "monospace" }}>2020-21 SPIKE = COVID REVERSE MIGRATION — HIGHEST EVER DEMAND · 389 CRORE PERSON-DAYS</div>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={NATIONAL_TREND} margin={{ top: 0, right: 20, left: 20, bottom: 0 }}>
                ...
                <Bar dataKey="personDays" name="Person-days (crore)" radius={[2, 2, 0, 0]}>
                  {NATIONAL_TREND.map((d, i) => <Cell key={i} fill={d.year === "2020-21" ? "#ffaa00" : "#88ffcc55"} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            <div style={{ marginTop: 32 }}>
              <div style={{ fontSize: 11, color: "#444", letterSpacing: 2, marginBottom: 20 }}>BUDGET ALLOCATION VS EXPENDITURE · ₹ CRORE</div>
              ...
              <div style={{ marginTop: 16, padding: "12px 20px", background: "#001a0f", border: "1px solid #003322", fontFamily: "monospace", fontSize: 10, color: "#448866", lineHeight: 1.8 }}>
                &#9685; &nbsp;In 2023-24, expenditure (₹1,05,450 Cr) exceeded budget allocation (₹60,000 Cr) significantly. &nbsp;Result: 20 states ran into negative fund balance — workers did the work but payment was delayed.
              </div>
            </div>
          </>
        )}
        ...
        {tab === "states" && (
          <div ref={chartRef}>
            <ChartToolbar chartRef={chartRef} data={STATE_DAYS_DATA} csvFilename="nrega-state-data" />
            <div style={{ fontSize: 11, color: "#444", letterSpacing: 2, marginBottom: 6 }}>AVERAGE DAYS OF EMPLOYMENT PER HOUSEHOLD · STATE-WISE · 2023-24</div>
            <div style={{ fontSize: 10, color: "#224433", marginBottom: 20, fontFamily: "monospace" }}>100 DAYS GUARANTEED · NATIONAL AVG: ~47 DAYS · GREEN ≥ 50 · AMBER 35-50 · RED &lt; 35</div>
              ...
              <BarChart data={STATE_DAYS_DATA} margin={{ top: 0, right: 20, left: 20, bottom: 80 }}>
                ...
                <Bar dataKey="days" name="Avg days/household" radius={[2, 2, 0, 0]}>
                  {STATE_DAYS_DATA.map((d, i) => (
                    <Cell key={i} fill={highlightedState ? (d.state === highlightedState ? d.color : "#1a1a1a") : d.color} fillOpacity={0.8} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            <div style={{ marginTop: 20, padding: "14px 20px", background: "#001a0f", border: "1px solid #003322", fontFamily: "monospace", fontSize: 11, color: "#448866", lineHeight: 1.8 }}>
              &#9685; &nbsp;No state has achieved the 100-day guarantee on average. &nbsp;Rajasthan leads at 67 days — still only 67% of what is legally mandated. &nbsp;Haryana and Punjab, despite being prosperous states, show the lowest utilisation.
            </div>
          </div>
        )}
        ...
        {tab === "issues" && (
          <>
            <div style={{ fontSize: 11, color: "#444", letterSpacing: 2, marginBottom: 24 }}>DELIVERY GAP SCORECARD · WHERE THE GUARANTEE FAILS</div>
            {KEY_ISSUES.map((item, i) => (
              <div key={i} style={{ marginBottom: 24, padding: "20px 24px", background: "#0a0a0a", borderLeft: `3px solid ${item.color}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10, fontFamily: "'IBM Plex Mono', monospace" }}>
                  <span style={{ fontSize: 12, color: "#ccc" }}>{item.issue}</span>
                  <span style={{ fontSize: 11, color: "#555" }}>Actual: <span style={{ color: item.color }}>{item.actual}</span> &nbsp;/ Guaranteed: <span style={{ color: "#888" }}>{item.guaranteed}</span></span>
                </div>
                <div style={{ height: 8, background: "#111", borderRadius: 4, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${(item.actual / item.guaranteed) * 100}%`, background: item.color, borderRadius: 4, transition: "width 0.8s ease" }} />
                </div>
                <div style={{ fontSize: 10, color: "#333", marginTop: 6, fontFamily: "monospace" }}>{((item.actual / item.guaranteed) * 100).toFixed(0)}% of guarantee fulfilled</div>
              </div>
            ))}
            <div style={{ padding: "20px 24px", background: "#0a0000", border: "1px solid #330000", fontFamily: "monospace", fontSize: 11, color: "#884444", lineHeight: 1.8, marginTop: 8 }}>
              <div style={{ color: "#ff4444", fontSize: 12, marginBottom: 8, fontFamily: "'IBM Plex Mono', monospace" }}>⚠ THE JOB CARD DELETION CRISIS</div>
              1.46 crore workers were deleted from NREGA job cards in 2022-23 and 2023-24. &nbsp;LibTech India found ~15% of these deletions in Andhra Pradesh were wrongful. &nbsp;Extrapolated nationally: ~22 lakh genuine workers may have lost access.<br />
              Since Jan 1 2024, Aadhaar-Based Payment System is mandatory. &nbsp;27.4% of all registered workers are not Aadhaar-linked — they cannot be paid. &nbsp;Their work is real. Their wages are withheld by a system requirement.
            </div>
          </>
        )}
        ...
        {tab === "workers" && (
          <>
            <div style={{ fontSize: 11, color: "#444", letterSpacing: 2, marginBottom: 20 }}>WHO ACTUALLY WORKS UNDER NREGA · PARTICIPATION BREAKDOWN · 2022-23</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
              <div>
                {PARTICIPATION_DATA.map((d, i) => (
                  <div key={i} style={{ marginBottom: 20 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontFamily: "'IBM Plex Mono', monospace", fontSize: 12 }}>
                ...
                <div style={{ marginTop: 24, padding: "14px 16px", background: "#001a0f", border: "1px solid #003322", fontFamily: "monospace", fontSize: 10, color: "#448866", lineHeight: 1.8 }}>
                  &#9685; Women make up 56% of NREGA workers — higher than the 33% legal minimum. &nbsp;For many rural women this is their only source of independent income. &nbsp;Kerala has 91% women participation — highest in India.
                </div>
              </div>
              <div>
                <div style={{ fontSize: 10, color: "#444", letterSpacing: 2, marginBottom: 16 }}>WOMEN PARTICIPATION % BY STATE · 2023-24</div>
                <ResponsiveContainer width="100%" height={380}>
                  <BarChart data={[...STATE_DAYS_DATA].sort((a, b) => b.women_pct - a.women_pct)} layout="vertical" margin={{ top: 0, right: 20, left: 120, bottom: 0 }}>
                    ...
                    <Tooltip contentStyle={{ background: "#0d0d0d", border: "1px solid #ff88aa", fontFamily: "monospace", fontSize: 11 }} cursor={{ fill: "#111" }} formatter={(v) => [`${v}%`, "Women workers"]} />
